...
def init_database():
    ...
    # Tabela de progresso - DEFINIDA CORRETAMENTE
    c.execute('''
...
def iniciar_desafios_semanais(usuario_id):
    ...
    c.execute("""
        ...
        AND date(substr(data_inicio, 7, 4) || '-' || substr(data_inicio, 4, 2) || '-' || substr(data_inicio, 1, 2)) >= date('now', '-7 days')
    """, (usuario_id,))
...
def verificar_premios_especiais(usuario_id):
    ...
    if melhor_semana and completados > melhor_semana[0]:
